...
y_matrix = eye(num_labels)(y,:); % expand the y vector into a matrix

% first activation layer is the input; add column of bias 1's to beginning
a1 = [ones(size(X, 1), 1) X];
z2 = a1*Theta1';
% add column of bias 1's to beginning
a2 = [ones(size(a1, 1), 1) sigmoid(z2)];
z3 = a2*Theta2';
a3 = sigmoid(z3);

% Get rid of first column in thetas; take all rows with :, start at col 2
Theta1Reg = Theta1(:,2:size(Theta1,2));
Theta2Reg = Theta2(:,2:size(Theta2,2));

% Dont forget to double sum
...
Theta2_grad = d2_accum ./ m;
Theta1_grad = d1_accum ./ m;
